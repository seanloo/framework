...
//        $request = $this->app['request'];
        ...
        // 获取模版名称
        $theme = $this->app->env->get('app.theme', 'default');
...
//        $depr = $this->config['view_depr'];
        ...
        if (!$this->config['view_mode']) {
            ...
            if (isset($app)) {
                $view = $this->config['view_dir_name'];
                $viewPath = $this->app->getBasePath() . $theme . DIRECTORY_SEPARATOR . $app . DIRECTORY_SEPARATOR . $view . DIRECTORY_SEPARATOR;

                if (is_dir($viewPath)) {
                    $path = $viewPath;
                } else {
                    $path = $this->app->getRootPath() . $view . DIRECTORY_SEPARATOR . $theme . DIRECTORY_SEPARATOR . $app . DIRECTORY_SEPARATOR;
                }

            } else {
                $path = $this->config['view_path'] . $theme . DIRECTORY_SEPARATOR . ($appName ? $appName . DIRECTORY_SEPARATOR : '');
            }
...
//                    }
//                }
//                $template = $module . $depr . $template;
//            } else {
//                $template = str_replace(['/', ':'], $depr, substr($template, 1));
//            }
        } else {
            $path = $this->config['view_path'] . $theme . DIRECTORY_SEPARATOR;
        }
        $this->template->view_path = $path;
