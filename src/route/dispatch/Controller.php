...
class Controller extends Dispatch
{
    ...
    public function init(App $app)
    {
        ...
        // 获取模块名
        $module = strip_tags($result[0] ?: $this->rule->config('default_module'));
        ...
        // 获取操作名
        $actionName = $result[2] ?? $this->rule->config('default_action');
        $this->actionName = strip_tags($actionName);
    ...
    public function controller(string $name)
    {
        ...
        $class = $this->app->parseClass($controllerLayer, $this->module . '/' . $name . $suffix);

        if (class_exists($class)) {
            return $this->app->make($class, [], true);
        } elseif ($emptyController && class_exists($emptyClass = $this->app->parseClass($controllerLayer, $this->module . '/' . $emptyController . $suffix))) {
